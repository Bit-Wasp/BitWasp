<pre><?php

/**
 * Bitcoin Test Controller
 * 
 * This controller is used to test out some bitcoin functions before
 * adding them to the live code. Also has a blockexplorer feature.
 */
 
class Bitcoin_Test extends CI_Controller {
	
	public function __construct() {
		parent::__construct();
		$this->load->library('bw_bitcoin');
		$this->load->library('Raw_transaction');
		
	}
	
	public function key() {
		$this->load->library('totp');
		$this->load->library('ciqrcode');
		
		$discrepancy = 2;
		
		$secret = $this->totp->createSecret();
		$url = $this->totp->getOTPLink('Bitwasp', $secret);
		$qr = $this->ciqrcode->generate_base64(array('data' => $url));
		$ts = time();
		
		
		echo "Secret: $secret <br />";
		echo "URL   : $url <br />";
		echo "<img src='data:image/png;base64,{$qr}'><br />";
		date_default_timezone_set("UTC");
		
		echo $ts."<br />";
		$code = $this->totp->getCode($secret);
		echo "$code<br />";
		$check = $this->totp->verifyCode($secret, $code, 2);    // 2 = 2*30sec clock tolerance
		if ($check) {
			echo 'OK';
		} else {
			echo 'FAILED';
		}

	}
	
	public function decode() {
		$d = Raw_transaction::decode('');
		var_dump($d);
		print_r($d);
	}
};







