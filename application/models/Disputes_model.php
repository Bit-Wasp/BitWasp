...
class Disputes_model extends CI_Model {
	...
	public function get($dispute_id) {
		$this->db->where('id', $dispute_id);
		$query = $this->db->get('disputes');
		if($query->num_rows() > 0) {
			$row = $query->row_array();
			
			return $this->create_dispute_info_array($row);
		} else {
			return FALSE;
		}
	}
	...
	public function create_dispute_info_array($row) {
		...
		$this->db->where('id', $row['disputing_user_id']);
	...
	public function create($array) {
		return ($this->db->insert('disputes', $array) == TRUE) ? TRUE : FALSE;
	}
	...
	public function set_final_response($order_id) {
		$this->db->where('order_id', $order_id);
		return ($this->db->update('disputes', array('final_response' => '1')) == TRUE) ? TRUE : FALSE;
	}
	...
	public function post_dispute_update($array) {
		$array['time'] = time();
		return ($this->db->insert('disputes_updates', $array) == TRUE) ? TRUE : FALSE;
	}
